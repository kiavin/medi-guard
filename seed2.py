...
# Your active clinician JWT Token
HEADERS = {
    "Content-Type": "application/json",
    "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2YjY4ZjUwOC1iNTEzLTQzZTAtYWU2MS03OTY0ZDk5YmM0YzYiLCJyb2xlIjoiY2xpbmljaWFuIiwiZXhwIjoxNzc2NzgyNzkxfQ.XhEzLn27j4fmTdVoriXHVLVJVZabAkrLkzioZakUJwY"
}

def create_patient(data):
    print(f"Creating patient: {data['first_name']} {data['last_name']}...")
    # FIXED: Updated endpoint to /patients/register to match your API specs
    res = requests.post(f"{BASE_URL}/patients/register", json=data, headers=HEADERS)
    if res.status_code == 201:
        return res.json()["dataPayload"]["data"]["id"]
    print(f"Error creating patient ({res.status_code}): {res.text}")
    return None
...
def open_consultation(data):
    ...
    if res.status_code == 201:
        return res.json()["dataPayload"]["data"]["id"]
    print(f"Error opening consultation ({res.status_code}): {res.text}")
    return None
...
def close_consultation(consult_id, data):
    ...
    if res.status_code != 200:
        print(f"Error closing consultation ({res.status_code}): {res.text}")
