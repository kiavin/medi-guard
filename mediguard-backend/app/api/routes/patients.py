...
from app.schemas.patient import (
    ...
    PatientConsultationSummaryResponse,
    ClinicalSummary,
    ActiveAlert,
...
def register_patient(
    ...
    """
    existing_patient = (
        db.query(Patient).filter(Patient.national_id == patient.national_id).first()
    )
    if existing_patient:
        raise HTTPException(
...
def get_patients(
    ...
    """
    Retrieve a list of all patients with search, sort, and pagination.
    """
    ...
    result = apply_filters_and_paginate(
        ...
        model=Patient,
        params=params,
        search_columns=["first_name", "last_name", "national_id"],
    )

    # Serialize items using the Pydantic schema
    result["items"] = [
        PatientResponse.model_validate(item).model_dump() for item in result["items"]
    ]
...
def generate_patient_excel_export(user_email: str):
    ...
    try:
        ...
        filename = (
            f"exports/patient_roster_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
            ...
            cons_data.append(
                {
                    ...
                    "AI Diagnosis": (
                        c.prediction.primary_disease if c.prediction else "Not Run"
                    ),
...
def export_single_patient(
    ...
    background_tasks.add_task(
        generate_single_patient_export, patient_id, current_user.email
    )
...
@router.get(
    "/{patient_id}/consultation-summary",
    response_model=APIResponse[PatientConsultationSummaryResponse],
)
def get_patient_consultation_summary(
    ...
    patient = (
        db.query(Patient)
        .options(selectinload(Patient.consultations), selectinload(Patient.medications))
        .filter(Patient.id == patient_id)
        .first()
    )
    ...
    sorted_consultations = sorted(
        patient.consultations, key=lambda x: x.consultation_date, reverse=True
    )
    last_consultation = sorted_consultations[0] if sorted_consultations else None

    # 3. Calculate metrics
    active_meds = [
        m for m in patient.medications if getattr(m, "status", "active") == "active"
    ]
    ...
    if last_consultation and last_consultation.final_diagnosis:
        ...
        recurrence_count = sum(
            1 for c in sorted_consultations if c.final_diagnosis == recent_diagnosis
        )
    ...
    summary = ClinicalSummary(
        last_visit_date=(
            last_consultation.consultation_date if last_consultation else None
        ),
        last_diagnosis=(
            last_consultation.final_diagnosis if last_consultation else "None Recorded"
        ),
    ...
    response_data = {
        "id": patient.id,
        "patient_number": (
            f"PT-{patient.national_id[-4:]}"
            if patient.national_id
            else f"PT-{patient.id[:4].upper()}"
        ),
...
@router.get(
    "/{patient_id}/medications", response_model=APIResponse[List[MedicationHistoryItem]]
)
def get_patient_medication_history(
    ...
    patient = db.query(Patient).filter(patient_id == patient_id).first()
    ...
    medications = (
        ...
        .order_by(Medication.prescribed_date.desc(), Medication.created_at.desc())
        .all()
    )

    return send_success(
        data=medications, message="Medication history retrieved successfully."
    )
